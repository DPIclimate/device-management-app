...
import { APIGatewayResponse } from '../types/APIResponseTypes';
import * as Linking from 'expo-linking';

interface useGatewaysResponse{
	response:APIGatewayResponse[]|undefined, //Response type depends on endpoint given to useFetch 
	isLoading:boolean,
	error:string|null,
	retry():void
}

export const useGateways = ():useGatewaysResponse =>{
	const [response, set_response] = useState<APIGatewayResponse[]>();
	...
	useEffect(() => {
        ...
        const url = `${state.application_server}/api/v3/gateways?field_mask=name,description`
		...
		const fetchData = async():Promise<void> =>{
			...
			if (state.network_status){
				...
				const store_response=await get_req_from_storage(path) as APIGatewayResponse[]
