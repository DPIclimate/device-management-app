...
export default function NearbyDevices() {
  ...
  const [userLocation, setLoaction] = useState(null)
    ...
    return(
            <Card>
                <TouchableOpacity style={{flexDirection:'row', alignItems:'center', justifyContent:'space-between', width:'100%', height:30}} onPress={() => handlePress(item)}>
                    <Text style={globalStyles.text, {flex:1}}>{item.ids.device_id}</Text>
                    <Text style={globalStyles.text, {flex:1}}>Dist: {item.loc_difference.toFixed(2)}km</Text>
  ...
  return (
    ...
    {console.log('this is data', data?.length)}
      {data?.length == 0 || !data ?
        ...
        />
        :
        <View style={{alignItems:'center', height:'100%', width:'100%'}}><Text>No devices nearby{console.log('here')}</Text></View>
      }
